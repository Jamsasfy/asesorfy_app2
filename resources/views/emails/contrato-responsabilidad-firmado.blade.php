...
<div class="container">
  ...
  <div class="content">
    ...
    <div class="data-box">
      <div><span>Descripción:</span> <strong>{{ $contrato->descripcion }}</strong></div>
      <div><span>Fecha de firma:</span> <strong>{{ $contrato->signed_at->format('d/m/Y H:i') }}</strong></div>
      <div><span>IP de firma:</span> <strong>{{ $contrato->ip_firma }}</strong></div>
      <div><span>Email:</span> <strong>{{ $contrato->email_firma }}</strong></div>
      <div style="word-break:break-all;"><span>Hash SHA256:</span> <strong style="font-family:monospace;font-size:12px;">{{ $contrato->hash_firma }}</strong></div>
    </div>

    <div class="highlight">
      <strong>📎 Documento adjunto:</strong><br>
      Encontrarás el documento firmado en PDF adjunto a este correo. Consérvalo como justificante.
    </div>
