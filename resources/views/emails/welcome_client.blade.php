...
<body>
@php
    // modo: 'transfer_pendiente' | 'welcome'
    $modo = $modo ?? 'welcome';
@endphp
...
<div class="container">
    ...
    <div class="content">
        ...
        @if($modo === 'transfer_pendiente')
            <p>
                Gracias por confiar en <strong>{{ config('app.name') }}</strong>.
                Hemos recibido tu contrato correctamente.
            </p>

            <div class="note-box">
                ℹ️ Has elegido pagar el importe inicial por <strong>transferencia</strong>.
                En cuanto confirmemos la recepción, activaremos el servicio y tu asesor/a se pondrá en contacto contigo.
            </div>
        @else
            <p>
                Gracias por confiar en <strong>{{ config('app.name') }}</strong>.
                Tu contratación está correcta y empezamos contigo el proceso de bienvenida como cliente.
            </p>
        @endif
