...
<div class="container">
  ...
  <div class="content">
    ...
    <p>Hola <strong>{{ $comercial->name }}</strong>, uno de tus clientes acaba de completar el pago.</p>

    <div class="highlight">
      <strong>✅ La venta ha quedado cerrada correctamente.</strong>
    </div>

    <div class="data-box">
      <div><span>Cliente:</span> <strong>{{ $venta->cliente->razon_social ?? '—' }}</strong></div>
      <div><span>Venta #:</span> <strong>{{ $venta->id }}</strong></div>
      <div><span>Importe:</span> <strong>{{ number_format($venta->importe_total, 2, ',', '.') }} € (base)</strong></div>
      <div><span>Fecha:</span> <strong>{{ now()->format('d/m/Y H:i') }}</strong></div>
    </div>

    <p style="text-align:center; margin:22px 0 10px;">
      <a href="{{ route('filament.admin.resources.ventas.view', ['record' => $venta->id]) }}" class="btn">
        Ver Venta
      </a>
    </p>
  </div>
