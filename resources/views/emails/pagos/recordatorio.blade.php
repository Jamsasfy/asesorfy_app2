...
<head>
    ...
    <title>Recordatorio de Pago - {{ config('app.name') }}</title>
    <style>
...
<body>
    <div class="container">
        ...
        @php
            ...
            if (empty($nombresServicios)) $nombresServicios = 'servicio contratado';
        ...
        <div class="content">
            ...
            <p>
                Te recordamos que la activación del servicio de <strong>{{ $nombresServicios }}</strong> está pendiente de pago.
            </p>
            ...
            @if($venta->pago_inicial_metodo === 'transferencia')
                <div class="transfer-box">
                    ...
                    <p style="text-align: center; margin-bottom: 20px; font-size: 14px; color: #3b82f6;">
                        Puedes realizar el ingreso del importe pendiente aquí:
                    </p>
                    ...
                    <p style="text-align: center; font-size: 12px; color: #64748b; margin-top: 15px;">* Envíanos el justificante respondiendo a este email.</p>
                </div>

            {{-- CASO 2: TARJETA --}}
            @else
                <div class="pay-box">
                    <h3 style="margin-top: 0; color: #9a3412;">Finalizar Pago</h3>
                    <p style="margin-bottom: 10px; color: #7c2d12;">Para activar el servicio, abona el importe de forma segura:</p>
                    <div style="font-size: 26px; font-weight: 800; color: #0f172a; margin: 15px 0;">
                        {{ number_format($totalPagar, 2, ',', '.') }} €
                    </div>
                    <a href="{{ route('payment.pay', ['venta' => $venta->id]) }}" class="pay-btn">💳 Pagar Ahora</a>
                </div>
            @endif

            <p style="margin-top: 30px;">Si ya has realizado el pago, ignora este mensaje o envíanos el justificante.</p>
        </div>
